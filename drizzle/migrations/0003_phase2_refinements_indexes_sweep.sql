...
-- score refresh; the feed offers no webhooks and the cost trade-off was reviewed with the user.

CREATE INDEX IF NOT EXISTS idx_schedule_items_india_date
  ON public.schedule_items (has_india, date_ist);
CREATE INDEX IF NOT EXISTS idx_schedule_items_india_start
  ON public.schedule_items (has_india, start_time);
CREATE INDEX IF NOT EXISTS idx_schedule_items_live
  ON public.schedule_items (is_live) WHERE is_live;
CREATE INDEX IF NOT EXISTS idx_india_results_sport_res
  ON public.india_results (sport_code, res_code);
CREATE INDEX IF NOT EXISTS idx_india_results_res
  ON public.india_results (res_code);
